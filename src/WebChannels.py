...
class WebChannels(object):
	...
	def	request(self):
		...
		getPage(PROXY+url, agent=USER_AGENT, headers={'Content-Type':'application/x-www-form-urlencoded'}).addCallback(self.__callback).addErrback(self.__errback)

	def request_and_return(self):
		print "[SP] request_and_return webpage.."
		url = "http://www.wunschliste.de/updates/stationen"
		from plugin import PROXY, USER_AGENT
		req = Request(PROXY+url, agent=USER_AGENT, headers={'Content-Type':'application/x-www-form-urlencoded'})
		try:
